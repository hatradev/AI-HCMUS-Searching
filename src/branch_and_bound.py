...
def upper_bound(index, current_weight, current_value):
    ...
    sorted_indices = sorted(
        range(index, n), key=lambda i: Items_value[i] / Items_weight[i], reverse=True
    )
...
def knapSack_recursive_BNB(
    current_value, current_weight, index, selected_items, max_value, max_items
):
    if index == n:
    ...
    if (
        bound <= max_value
    ):  # Prune the branch if bound is not better than current max_value
        return max_value, max_items

    # Try not selecting the current item
    max_value, max_items = knapSack_recursive_BNB(
        current_value, current_weight, index + 1, selected_items, max_value, max_items
    )
    # Try selecting the current item
    if (
        current_weight + Items_weight[index] <= Total_weight
        and selected_items[index] == 0
    ):
...
file = open(
    "C:/Users/Admin/Desktop/AI/ArtificialIntelligence_21CLC10/datasets/INPUT_2.txt"
)
...
max_value, max_items = knapSack_recursive_BNB(
    0, 0, 0, selected_items, max_value, max_items
)
